...
# Define schema for game data (same as before)
schema = StructType([
...
class CardRecommender:
    ...
    # Previous methods remain the same until prepare_training_data
    ...
    def save_model_to_mongodb(self, mongo_client):
        """Save the trained model and associated data to MongoDB."""
        try:
            db = mongo_client[MONGO_DATABASE]
            model_collection = db[MODEL_COLLECTION]
            
            # Convert Spark DataFrames to pandas for storage
            model_path = f"/tmp/als_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            print(f"Model successfully saved to MongoDB with path: {model_path}")
...
class CardRecommendationService:
    ...
    def load_latest_model(self):
        """Load the most recent model from MongoDB."""
        try:
            ...
            if model_data:
                # Load ALS model
                self.model = ALS.load(model_data['model_path'])
...
# Signal handlers and main execution remain the same
...
if __name__ == "__main__":
    ...
    # Set up streaming (same as before)
